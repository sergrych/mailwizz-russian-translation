 ...
 */
 ...
 return array (
  ...
  'The campaign name, this is used internally so that you can differentiate between the campaigns. Will not be shown to subscribers.' => 'Название кампании, оно используется внутри, чтобы вы могли различать кампании. Не будет показано подписчикам.',
  'The list from where we will pick the subscribers. We will send to all the confirmed subscribers if no segment is specified.' => 'Список, из которого мы будем выбирать подписчиков. Мы отправим всем подтвержденным подписчикам, если сегмент не указан.',
  ...
  'In most of the cases, you will want to keep these fields as hidden fields.' => 'В большинстве случаев вы захотите оставить эти поля скрытыми.',
  ...
  'Change subscriber custom field value upon campaign open' => 'Изменить значение настраиваемого поля подписчика при открытии кампании',
  'This is useful if you later need to segment your list and find out who opened this campaign or who did not and based on that to take another action, like sending the campaign again to subscribers that did not open it previously.' => 'Это полезно, если вам позже потребуется сегментировать свой список и выяснить, кто открыл эту кампанию, а кто нет, и на основании этого предпринять другое действие, например, повторно отправить кампанию подписчикам, которые ранее не открывали ее.',
  'Actions against subscribers upon campaign open' => 'Действие с подписчиком при открытии компании',
  'When a subscriber opens your campaign, do following actions against the subscriber itself:' => 'Когда подписчик открывает вашу кампанию, выполните следующие действия с самим подписчиком:',
  'Campaign delivery servers' => 'Сервера доставки компании',
  'Select which delivery servers are used for this campaign, if no option is selected, all the available servers will be used.' => 'Выберите, какие серверы доставки будут использоваться для этой кампании. Если не выбран ни один параметр, будут использованы все доступные серверы.',
  ...
  'Update campaign' => 'Обновление компании',
  ...
  'Show more options' => 'Показать дополнительные настройки',
  'Show less options' => 'Скрыть дополнительные настройки',
  'Following tags are available to be used as dynamic values:' => 'Следующие теги доступны для использования в качестве динамических значений:',
  'Description' => 'Описание',
  'Increment the value by X where X is an integer' => 'Увеличьте значение на X, где X - целое число',
  'Decrement the value by X where X is an integer' => 'Уменьшить значение на X, где X - целое число',
  'Multiply the value by X where X is an integer' => 'Умножьте значение на X, где X - целое число',
  'Set current date and time, in Y-m-d H:i:s format' => 'Установите текущую дату и время в формате Y-m-d H: i: s',
  'Set current date, in Y-m-d format' => 'Установить текущую дату в формате Y-m-d',
  'Set the current IP address' => 'Установите текущий IP-адрес',
  'Set the current User Agent string' => 'Установить текущую строку User Agent',
  'Change subscriber custom field value upon campaign sent' => 'Change subscriber custom field value upon campaign sent',
  'Extra tags' => 'Дополниельные теги',
  'This allows you to define extra campaign tags which you might use in the campaign.' => 'Это позволяет вам определять дополнительные теги, которые вы можете использовать в кампании.',
  'Please note that your tags defined here will be prefixed with the prefix: {prefix}' => 'Обратите внимание, что ваши теги, определенные здесь, будут иметь префикс: {prefix}',
  'So if you define a tag named COMPANY_NAME, you can then use it like: "{example}" in your campaign.' => 'Поэтому, если вы определите тег с именем COMPANY_NAME, вы можете использовать его следующим образом: "{example}" в вашей компании.',
  ...
  'Previous opened/unopened campaigns' => 'Предыдущие открытые / неоткрытые кампании',
  'Send this campaign only to subscribers that have opened or have not opened previous campaigns:' => 'Отправляйте эту кампанию только подписчикам, которые открыли или не открыли предыдущие кампании:',
  ...
  'Actions against subscribers upon campaign sent' => 'Действия с подписчиками при отправке кампании',
